...
def create_strategy_coin_diagram(message: types.Message, data: pd.DataFrame, bot_name: str) -> os.path:
    """Функция построения диаграммы типа бот-стратегия.

    :param message: Сообщение от пользователя.
    :param data: Данные для построения.
    :param bot_name: Название столбца из CPU.
    :return: Путь к диаграмме на компьютере.
    """

    fig, ax = plt.subplots(figsize=(15, 48))

    clear_df = data[bot_name].where(lambda x: x != 0.0).dropna()
    ...
    plt.savefig(os.path.join(path_to_save, f'{bot_name}.png'))
    plt.close('all')
    return os.path.join(path_to_save, f'{bot_name}.png')
